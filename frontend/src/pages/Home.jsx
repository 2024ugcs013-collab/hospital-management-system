...
export default function Home() {
  ...
  useEffect(() => { getDoctors().then(setDoctors).catch(() => setDoctors([])); }, []);
  return (
      ...
      <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        ...
        <section id="doctors" className="mt-20">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {doctors.map((doctor) => (
              <article key={doctor._id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                <div className="flex items-start gap-4">
                  <div className="grid h-16 w-16 place-items-center rounded-2xl bg-slate-100 text-2xl">👩‍⚕️</div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-semibold text-brand-700">{doctor.specialization}</span>
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{doctor.experience}</span>
                    </div>
                    <h3 className="mt-3 text-lg font-semibold text-slate-950">Dr. {doctor.userId?.name}</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-600">₹{doctor.consultationFee} consultation fee</p>
                  </div>
                </div>
            ...
            {!doctors.length && <p className="text-slate-500">No verified doctors are available right now.</p>}
          </div>
        </section>
